"""One real Run, end to end, against a planted bug. It spends Z.AI credits, so it is opt in."""
...
def test_a_real_run_produces_a_valid_report(
    ...
    assert main(["review", "--timeout", "6"]) == 0
    ...
    with capsys.disabled():
        print(f"\n{report.stats.seconds:g}s, {len(report.findings)} findings, score {report.score}")
