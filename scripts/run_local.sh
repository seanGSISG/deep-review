...
#   AGENT=opencode|pi scripts/run_local.sh <owner/repo> <pr-number> [out-dir] [model]
# Needs: gh (authed), uv, opencode or pi, ZHIPU_API_KEY (or Z_AI_API_KEY).
set -euo pipefail
REPO=$1; PR=$2; OUT=${3:-./bakeoff}; AGENT=${AGENT:-opencode}
MODEL=${4:-$([ "$AGENT" = pi ] && echo zai/glm-5.3 || echo zai-coding-plan/glm-5.3)}
TOOLS=$(cd "$(dirname "$0")/.." && pwd)
export ZHIPU_API_KEY=${ZHIPU_API_KEY:-${Z_AI_API_KEY:?set ZHIPU_API_KEY}}; export ZAI_API_KEY=$ZHIPU_API_KEY
SLUG=$(echo "$REPO-$PR-$AGENT" | tr '/' '-'); WORK=$(mktemp -d); RES="$OUT/$SLUG"; mkdir -p "$RES"
...
if [ "$AGENT" = pi ]; then
  timeout "${AGENT_TIMEOUT:-1200}" pi -p --mode json --no-session --no-extensions --no-skills --no-prompt-templates \
    --model "$MODEL" --thinking "${THINKING:-medium}" "$(cat .deep-review/prompt.md)" < /dev/null \
    > .deep-review/agent-events.jsonl 2> .deep-review/agent.err || echo "[$SLUG] agent exit $?"
else
  ...
  timeout "${AGENT_TIMEOUT:-1200}" $OC run --format json --pure --dangerously-skip-permissions -m "$MODEL" \
    --title "deep-review $SLUG" "$(cat .deep-review/prompt.md)" < /dev/null \
    > .deep-review/agent-events.jsonl 2> .deep-review/agent.err || echo "[$SLUG] agent exit $?"  # stdin must be closed or opencode blocks
fi
